...
if (isset($_POST['action']) && $_POST['action'] === 'race') {
    ...
    // -- Step 1: Build absolute URL for /var/www/hccbet/backend.php --
    // curl cannot resolve relative paths — it must have a full URL.
    $protocol   = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host       = $_SERVER['HTTP_HOST'];
    $dir        = rtrim(dirname($_SERVER['PHP_SELF']), '/');
    $backendUrl = $protocol . '://' . $host . $dir . '//var/www/hccbet/backend.php';

    $sessionCookie = session_name() . '=' . session_id();

    // Release the session lock before making curl requests to /var/www/hccbet/backend.php.
    // /var/www/hccbet/backend.php calls session_start() too — without this, it blocks waiting
    ...
    // -- Step 2: Report transaction result to /var/www/hccbet/backend.php and capture response --
