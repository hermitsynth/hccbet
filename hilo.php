...
if ($raw !== '') {
    ...
    if (isset($payload['action']) && $payload['action'] === 'play') {
        ...
        /* ── forward result to /var/www/hccbet/backend.php server-side (no client involvement) ── */
        $balanceMsg = '';

        // Release the session lock before making server-side HTTP calls to /var/www/hccbet/backend.php.
        // Without this, /var/www/hccbet/backend.php blocks forever waiting to acquire the same session lock
        ...
        // Step 1: fetch a verify token from /var/www/hccbet/backend.php
        $verifyToken = '';
        ...
        $backendUrl      = (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '//var/www/hccbet/backend.php';
        ...
        // Send the win/lose payload (with verify token) to /var/www/hccbet/backend.php
        ...
        if ($backendResponse !== false) {
            ...
            // FIX 1: /var/www/hccbet/backend.php returns "status", not "balance_msg"
            if (!empty($bd['status'])) {
    ...
    function play(side) {
        ...
        Promise.all([animPromise, serverPromise])
            .then(([_, data]) => {
                ...
                // FIX 2: populate #server-msg with the status message from /var/www/hccbet/backend.php
                if (data.balance_msg) {
